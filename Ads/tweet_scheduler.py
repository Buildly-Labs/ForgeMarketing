...
- Loads brand-aware tweet templates from `data/tweet_templates.json`

Set TWITTER_* env vars or pass --bearer-token.
"""
...
def default_content_path() -> Path:
    return PROJECT_ROOT / 'data' / 'tweet_content.json'
...
def load_content(path: Path) -> List[str]:
    if not path.exists():
        logger.warning('Tweet content file not found: %s', path)
        return []
    try:
        ...
        logger.error('Failed to load tweet content from %s: %s', path, exc)
    return []
...
def pick_brand_text(brand: str, templates: Dict[str, List[str]]) -> Optional[str]:
    if not templates:
        return None
    options = templates.get(brand.lower()) or templates.get('both') or []
    if not options:
        return None
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument('--content', type=Path, default=default_content_path(), help='JSON file with tweet texts')
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.text:
        ...
        texts = load_content(args.content)
    if not texts:
        logger.error('No tweet content available; add entries to %s or pass --text/--brand', args.content)
        return 1
